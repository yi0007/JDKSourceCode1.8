...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    /**
     * 扩容时用于存放数据的变量，转移的时候用的数组，扩容完成后会置为null。
     */
    ...
    /* ---------------- Public operations -------------- */
    // 都没有对存储Map元素Node的table变量进行初始化。而是在第一次put操作的时候在进行初始化。
     ...
     * （4）如果待插入的元素所在的桶不为空且 不在迁移元素，则锁住这个桶（分段锁）；
    ...
    private final void tryPresize(int size) {
        ...
        while ((sc = sizeCtl) >= 0) {
            ...
            if (tab == null || (n = tab.length) == 0) {
            ...
            } else if (c <= sc || n >= MAXIMUM_CAPACITY)//一直扩容到的c小于等于sizeCtl或者数组长度大于最大长度的时候，则退出，所以在一次扩容之后，不是原来长度的两倍，而是2的n次方倍
                break;
            else if (tab == table) {
                int rs = resizeStamp(n);
                /**
                ...
                if (sc < 0) {
                    ...
                    if (U.compareAndSwapInt(this, SIZECTL, sc, sc + 1))
                        ...
                        (rs << RESIZE_STAMP_SHIFT) + 2))// 没有在初始化或扩容，则开始扩容
                    transfer(tab, null);
            }
        }
    }
    ...
    private final void treeifyBin(Node<K, V>[] tab, int index) {
        ...
        int n, sc;
        if (tab != null) {
            if ((n = tab.length) < MIN_TREEIFY_CAPACITY)// 小于64
                tryPresize(n << 1);// 数组扩容
                        ...
                        for (Node<K, V> e = b; e != null; e = e.next) {
                            TreeNode<K, V> p =
                                    new TreeNode<K, V>(e.hash, e.key, e.val,
                                            null, null);
                            if ((p.prev = tl) == null)//把Node组成的链表，转化为TreeNode的链表，头结点仍然放在相同的位置
                                hd = p;//设置head
                            else
                                tl.next = p;
                            tl = p;
